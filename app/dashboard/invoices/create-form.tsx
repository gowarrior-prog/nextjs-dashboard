...
export default function Form({ customers }: { customers: any[] }) {
  ...
  return (
    <form action={formAction} className="bg-gray-50 p-6 rounded-lg border border-gray-200 text-gray-900">
      {/* Customer Name */}
      <div className="mb-4">
        <label htmlFor="customer" className="mb-2 block text-sm font-medium">Choose customer</label>
        <div className="relative">
          <select id="customer" name="customerId" className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm bg-white" aria-describedby="customer-error">
        ...
        <div id="customer-error" aria-live="polite" className="mt-2 text-sm text-red-500 font-medium">
      ...
      <div className="mb-4">
        ...
        <div className="relative mt-2 rounded-md">
          <input id="amount" name="amount" type="number" step="0.01" placeholder="Enter USD amount" className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm bg-white" aria-describedby="amount-error" />
          <CurrencyDollarIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500" />
        </div>
        {/* Amount Validation Feedback */}
        <div id="amount-error" aria-live="polite" className="mt-2 text-sm text-red-500 font-medium">
          {state.errors?.amount?.map((error: string) => <p key={error}>{error}</p>)}
        </div>
      </div>

      {/* Main Database Message Feedback */}
      {state.message && (
